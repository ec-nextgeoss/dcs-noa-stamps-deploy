function stamps_mc_header_nostart_allcpu(start_step,end_step,patches_flag,est_gamma_parm,patch_list_file)
...
if ~isempty(ix_split_patches)
    ...
    n_patches_core = floor(n_patches./n_cores);
    n_patches_remain = rem(n_patches,n_cores);
 
    % splitting of the patch list
    counter_cores = 0;
    for k=1:n_cores
        rem_patch = max(0,k+n_patches_remain-n_cores)
        ix = [(k-1)*n_patches_core+1+max(0,rem_patch-1):1:k*n_patches_core+rem_patch];
        ix(ix>n_patches) =[];
